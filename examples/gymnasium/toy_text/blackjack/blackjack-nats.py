"""
Solving Blackjack with Reinforcement Learning over NATS
=======================================================
Adapted by Glenn Lewis from:
...
# favorite discrete RL algorithm.
#
...
async def main():
    ...
    # Before setting up the environment, connect to a NATS JetStream:
    nc = await nats.connect("localhost")
    # Subscribe to 'gym.agent' for messages from agent:
    sub = await nc.subscribe("gym.agent.action")
    ...
    # game is called an episode) at a time.
    # The agent will have to experience a lot of episodes to
    ...
    for episode in tqdm(range(n_episodes)):
        obs, info = env.reset()
        msg = (obs, info)
        await nc.publish("gym.env.reset", json.dumps(msg).encode())
        done = False

        # play one episode
        while not done:
            # action = agent.get_action(obs)
            await nc.publish("gym.env.obs", json.dumps(obs).encode())
            ...
            msg = (obs, action, reward, terminated, next_obs)
            await nc.publish("gym.env.update", json.dumps(msg).encode())
        ...
        # agent.decay_epsilon()
        await nc.publish("gym.env.decay_epsilon")
